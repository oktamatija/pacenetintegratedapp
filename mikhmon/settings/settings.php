...
<div class="col-12">
	<div class="card">
        <div class="card-header">
            <h3 class="card-title">Billing Data</h3>
        </div>
    <div class="card-body">    
	<table class="table table-sm">
	<tr>
...
if (sessnameEl) {
    var sessX = sessnameEl.value;
    var pingBtn = document.getElementById('ping_test');
    if (pingBtn) {
        pingBtn.onclick = function(){ pingTest(sessX); };
    }
}
function closeX(){
    $('#pingX').hide();
}
var sesname = (document.settings && document.settings.sessname) ? document.settings.sessname : document.getElementById('sessname');
if (sesname) {
    function chksname(){
        if (sesname.value.toLowerCase() === 'mikhmon') {
