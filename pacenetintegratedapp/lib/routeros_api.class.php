...
class RouterosAPI
{
    ...
    var $timeout   = 1.5;   //  Connection attempt timeout and data read timeout (optimized for web API)
    var $attempts  = 1;     //  Single connection attempt to prevent blocking PHP-FPM
    var $delay     = 0;     //  No delay between attempts
...
function randNUC($length) {
	...
	$len = intval($length);
	if ($len < 2) {
		$charArray = str_split($all);
		$charCount = strlen($all);
		$result = "";
		for ($i = 1; $i <= $len; $i++) {
			$randChar = rand(0, $charCount - 1);
			$result .= $charArray[$randChar];
	...
	$nArr = str_split($nums);
	$uArr = str_split($upper);
	$allArr = str_split($all);

	$selected = array();
	$selected[] = $nArr[rand(0, count($nArr) - 1)];
	$selected[] = $uArr[rand(0, count($uArr) - 1)];

	$remaining = $len - 2;
	for ($i = 0; $i < $remaining; $i++) {
		$selected[] = $allArr[rand(0, count($allArr) - 1)];
	}
...
function randNULC($length) {
	...
	$nArr = str_split($nums);
	$uArr = str_split($upper);
	$lArr = str_split($lower);
	$allArr = str_split($all);

	$selected = array();
	$selected[] = $nArr[rand(0, count($nArr) - 1)];
	$selected[] = $uArr[rand(0, count($uArr) - 1)];
	$selected[] = $lArr[rand(0, count($lArr) - 1)];
